...
The mean number of steps per day is **`r mean_ds`** and the median is **`r median_ds`**. 
...
```{r, echo=FALSE}
maxstepsint <-circadian[which(circadian$steps==max(circadian$steps)),]
maxint <- maxstepsint[1,1]
maxsteps <- maxstepsint[1,2]
```

Thus, the maximum average number of steps, `r maxsteps`, was recorded in the interval `r maxint`.  
...
The mean number of steps per day is **`r mean_cds`** and the median is **`r median_cds`**. This means that **imputing missing data causes the average and median number of steps per day to increase by about 1412 and 371 steps, respectively**. 
